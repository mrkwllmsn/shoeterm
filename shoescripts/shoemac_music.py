# shoemac_music - an iTunes-style Music player for the shoemac fake-Aqua desktop.
#
# The first shoemac app to drive a *real* background process: it plays actual
# audio files through an external CLI player (default `mpg321`, overridable with
# $SHOEMAC_PLAYER).  Two control strategies, picked automatically -- the "hybrid"
# design:
#
...
#     status lines come back on stdout, giving real elapsed time, true pause, a
#     seekable progress bar, and end-of-track detection.  One persistent process
#     is reused across tracks.
#   * GENERIC mode -- used for any other command (e.g. `ffplay -nodisp -autoexit`
#     or `cvlc --play-and-exit {}`).  We spawn `<cmd> <file>` as a plain child in
#     its own session; pause/resume is OS-level SIGSTOP/SIGCONT (works with *any*
#     player), stop/next is SIGTERM, elapsed time is tracked on our own wall
#     clock, and end-of-track is the child exiting.  No seek; duration unknown.
#
# Source of music: double-click an audio file in Finder (it routes here, like
# images route to Preview), and/or browse the filesystem in the window's own
# Pane-based list and double-click a track.  Auto-advance walks the audio files
# of the current folder in order.
#
# The host gives us two hooks this app relies on: `tick(win, d)` (polled ~1/s off
# the main loop's select timeout -> animate the progress bar + auto-advance) and
# `close(win)` (teardown -> kill/quit the child).  All cv.text() is plain ASCII
# (a vector `text` aborts the DCS at the first non-ASCII byte); names go through
# sanitize() and transport glyphs are drawn shapes, never Unicode.
...
from shoemac_ui import C, mix, lighten, darken, human, sanitize, Pane  # noqa: F401

WIN_W, WIN_H = 620, 440

# Extensions we treat as playable.  Exported so Finder can decide whether a
# double-clicked file opens here (mirrors imgview.is_image).
AUDIO_EXTS = (
...
def _wav_player_cmd():
    return (os.environ.get("SHOEMAC_WAV_PLAYER") or "aplay").strip() or "aplay"
...
# --------------------------------------------------------------------------- #
#  local palette (graphite music-app accents over the shared Aqua C palette)
# --------------------------------------------------------------------------- #
M = {
    "tb0":    "#fbfbfd",   # transport bar gradient, top
    "tb1":    "#d4d6db",   # transport bar gradient, bottom
    "tbsep":  "#a9adb4",   # hairline under the transport bar
    "lcd0":   "#cfe9d6",   # the time/title "LCD" display, top (faint green glass)
    "lcd1":   "#aacdb6",   # display, bottom
    "lcdbd":  "#7f9787",   # display bezel
    "lcdink": "#1f3326",   # display text
    "btn0":   "#fdfdfd",   # transport button gloss, top
    "btn1":   "#c9ccd2",   # button, bottom
    "btnbd":  "#9aa0a8",   # button outline
    "glyph":  "#3a3f47",   # transport glyph ink
    "glyphhi": "#1f6fe0",  # play glyph when playing (Aqua blue)
    "track0": "#d8dbe0",   # progress trough, top
    "track1": "#c2c6cd",   # trough, bottom
    "fill0":  "#54b0ff",   # progress fill, top
    "fill1":  "#1f7ae0",   # fill, bottom
    "knob":   "#ffffff",
    "knobbd": "#5f86b3",
    "rowalt": "#f4f6f9",   # zebra row tint
    "rowsep": "#e4e7ec",
    "noteink": "#2a3a52",  # audio-row note glyph
    "pop0":   "#fffef7",   # gear popover, top
    "pop1":   "#f0eedf",   # popover, bottom
    "popbd":  "#b9b39a",
}
...
# --------------------------------------------------------------------------- #
#  background player (hybrid remote / generic)
# --------------------------------------------------------------------------- #
def _player_cmd():
    return (os.environ.get("SHOEMAC_PLAYER") or "mpg321").strip() or "mpg321"
...
class Player:
    ...
    def __init__(self):
        ...
        # remote-control only the bare mpg321/mpg123 binary; any customisation
        # (extra args or a {} placeholder) means "play it the simple way".
    ...
    def _ensure_remote(self):
        if self.proc and self.proc.poll() is None:
            return True
        try:
            # mpg321/mpg123 print @F frame-status (and @P) to *stderr*; merge it
            # into stdout so our single non-blocking reader sees the lot.  The
            # trailing "-" is the required dummy arg for the remote interface.
    ...
    def _parse(self, line):
        ...
        if line.startswith("@P "):
            # mpg321/mpg123: 0=stopped (STOP or finished), 1=paused,
            # 2=playing (resumed), 3=song ended.
            n = line[3:].strip()
            if n in ("0", "3"):
                was_user = self._user_stop
                self._user_stop = False
                # 3 is always a natural end; 0 is a natural end unless *we* sent
                # STOP.  Either way, latch `ended` so the app auto-advances.
...
def mus_init(win):
    ...
    if start and os.path.isfile(start) and is_audio(start):
        # focus the track in the list, then play it
...
def _play_path(win, path):
    ...
    win.title = "Music - " + (sanitize(os.path.basename(path)) or "")
...
def mus_draw(cv, d, win):
    ...
    # body background (list area) -- light, the contract says fill our own bg
    cv.pen("#ffffff")
    ...
    for i in range(pane.top, min(len(pane.entries), pane.top + visible)):
        ...
        if is_now:
            cv.vgrad(lx, y, lw, rowh, C["sel0"], C["sel1"], 4)
            ink = C["seltxt"]
        else:
            if (i - pane.top) % 2:
                cv.pen(M["rowalt"])
        ...
        gx = lx + 16
        if e.is_dir:
            _folder_glyph(cv, gx, gy, "#f0fbff" if is_now else C["dir"])
...
def _draw_popover(cv, d, win, L):
    ...
    cv.text(px + 10, py + 48, "Set $SHOEMAC_PLAYER to change")
